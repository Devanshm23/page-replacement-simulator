...
//     vector<int> pageID;
//     int noOfBlocks = 100 * noOfPages;
//     int hotPages = max(1, noOfPages / 3); // simulate locality

//     for (int i = 0; i < noOfBlocks; i++) {
//         int r = rand() % 100;
//         int pid;
//         if (r < 80) // 80% accesses to hot pages
//             pid = (rand() % hotPages) + 1;
//         else
//             pid = (rand() % noOfPages) + 1;
//         pageID.push_back(pid);
//     }
...
process* process::createProcess(int noOfPages, int noOfRAMPages) {
    vector<int> pageID;
    int noOfBlocks = 200 * noOfPages;

    // Define hot set (70% of accesses) and cold set (30%)
    int hotSize = max(1, noOfPages / 3); // 1/3 of pages are "hot"
    int coldSize = noOfPages - hotSize;

    vector<int> hotPages, coldPages;
    for(int i = 1; i <= hotSize; i++) hotPages.push_back(i);
    for(int i = hotSize + 1; i <= noOfPages; i++) coldPages.push_back(i);

    for(int i = 0; i < noOfBlocks; i++) {
        int r = rand() % 100;
        if(r < 70 && !hotPages.empty()) {
            pageID.push_back(hotPages[rand() % hotPages.size()]);
        } else if(!coldPages.empty()) {
            pageID.push_back(coldPages[rand() % coldPages.size()]);
        }
    }
...
vector<int> LRU::processRAM(int noOfPages, int noOfRAMPages, vector<int> pageID) {
    int missCount = 0, total = (int)pageID.size();
    list<int> recency; // front = MRU, back = LRU
    unordered_map<int, list<int>::iterator> pos; // page -> iterator in list
    unordered_set<int> cache; // optional fast membership check (or use pos.count)

    for (int pg : pageID) {
        auto itMap = pos.find(pg);
        if (itMap != pos.end()) {
            // Hit: move node to front
            recency.splice(recency.begin(), recency, itMap->second);
        } else {
            // Miss
            missCount++;
            if ((int)pos.size() == noOfRAMPages) {
                // Evict LRU (back)
                int lruPage = recency.back();
                recency.pop_back();
                pos.erase(lruPage);
            }
            // Insert new page at front (MRU)
            recency.push_front(pg);
            pos[pg] = recency.begin();
        }
    }
    return {missCount, total};
}
...
vector<int> MRU::processRAM(int noOfPages, int noOfRAMPages, vector<int> pageID) {
    ...
    for (int pg : pageID) {
        auto itMap = pos.find(pg);
        if (itMap != pos.end()) {
            // Hit: move node to front (becomes MRU)
            recency.splice(recency.begin(), recency, itMap->second);
        } else {
            // Miss
            missCount++;
            if ((int)pos.size() == noOfRAMPages) {
                // Evict MRU (front)
                int mruPage = recency.front();
                recency.pop_front();
                pos.erase(mruPage);
            }
            // Insert new page at front (it is now MRU)
            recency.push_front(pg);
            pos[pg] = recency.begin();
        }
    }
